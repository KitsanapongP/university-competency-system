    ...
    const confirmAction = async () => {
        ...
        try {
            if (confirmation.type === 'assign') {
                ...
                setToast({ success: text(language, 'กำหนดแบบแผนการประเมินให้รุ่นนักศึกษาแล้ว กรุณาคำนวณคะแนนใหม่เพื่ออัปเดตผลลัพธ์', 'Template assigned to the student cohort. Recalculate scores to update results.'), error: '' });
                ...
                setToast({ success: text(language, 'เปลี่ยนแบบแผนการประเมินของรุ่นนักศึกษาแล้ว กรุณาคำนวณคะแนนใหม่เพื่ออัปเดตผลลัพธ์', 'Template assignment replaced. Recalculate scores to update results.'), error: '' });
            } else {
                await removeTemplateAssignment(confirmation.assignment.assignmentId, confirmation.reason);
                setToast({ success: text(language, 'ถอดแบบแผนการประเมินออกจากรุ่นนักศึกษาแล้ว กรุณาคำนวณคะแนนใหม่เพื่ออัปเดตผลลัพธ์', 'Template unassigned from the student cohort. Recalculate scores to update results.'), error: '' });
            }
                ...
                {assignmentTemplate?.replacementCandidates && (
                    <>
                        ...
                        {assignmentTemplate.replacementCandidates.length === 0 ? (
                            <div className="template-assignment-empty">{text(language, 'ไม่มีแบบแผนที่พร้อมใช้ในหลักสูตรนี้', 'No reusable assessment plan is available in this curriculum.')}</div>
                        ) : (
