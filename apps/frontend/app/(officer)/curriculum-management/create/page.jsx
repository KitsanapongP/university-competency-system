...
import {
  createCurriculumFromForm,
  fetchCurriculums,
  fetchFaculties,
  fetchGeneratedCurriculumCode,
  fetchMajors,
} from "../../../../lib/curriculum";
...
import {
  applyDraftCurriculumStructureImport,
      ...
      {steps.map((label, i) => {
        ...
        return (
          <React.Fragment key={n}>
            <div
              className={`course-step-item ${active ? "course-step-item--active" : ""} ${done ? "course-step-item--done" : ""}`}
            >
            ...
            {i < steps.length - 1 && (
              <div
                className={`course-step-line ${step >= i + 2 ? "course-step-line--done" : ""}`}
              />
            )}
          </React.Fragment>
        );
...
function selectedMajorName(form, majors, fallback, language = "th") {
  const selectedMajor = majors.find(
    (major) => String(major.majorId) === String(form.majorId),
  );
  if (language === "en") {
    return (
      selectedMajor?.nameEn ||
      selectedMajor?.nameTh ||
      fallback ||
      "selected major"
    );
  }
  return (
    selectedMajor?.nameTh || selectedMajor?.nameEn || fallback || "สาขาที่เลือก"
  );
}
...
function buildDuplicateCurriculumNameWarning(
  form,
  majors,
  curriculums,
  language = "th",
  ...
  const selectedMajor = majors.find(
    (major) => Number(major.majorId) === majorId,
  );
  const majorName =
    language === "en"
      ? selectedMajor?.nameEn ||
        selectedMajor?.nameTh ||
        duplicate.degreeNameEn ||
        duplicate.degreeName ||
        "selected major"
      : selectedMajor?.nameTh ||
        selectedMajor?.nameEn ||
        duplicate.degreeName ||
...
function Step1({
  ...
  const filteredMajors = form.facultyId
    ? majors.filter(
        (major) => String(major.facultyId) === String(form.facultyId),
      )
    : [];
  const selectedMajor = majors.find(
    (major) => String(major.majorId) === String(form.majorId),
  ...
  return (
    <div className="course-form-group">
      <div className="course-form-field">
        <label className="course-form-field__label">
          ชื่อหลักสูตร (ภาษาไทย)
          <span className="course-form-field__required">*</span>
        </label>
        <input
          className="course-form-field__input"
          value={form.nameTh}
          onChange={(e) =>
            updateForm((p) => ({ ...p, nameTh: e.target.value }))
          }
      ...
      <div className="course-form-field">
        ...
        <input
          ...
          value={form.nameEn}
          onChange={(e) =>
            updateForm((p) => ({ ...p, nameEn: e.target.value }))
          }
      ...
      <div className="course-form-row">
        <div className="course-form-field">
          ...
          <div className="course-form-field__generated-code" aria-live="polite">
            {generatedCodeLoading
              ? language === "en"
                ? "Generating curriculum code..."
                : "กำลังสร้างรหัสหลักสูตร..."
              : generatedCode ||
                (language === "en"
          ...
          {generatedCodeError && (
            <div className="course-form-field__error">{generatedCodeError}</div>
          )}
        </div>
        <div className="course-form-field">
          <label className="course-form-field__label">
            คณะ<span className="course-form-field__required">*</span>
          </label>
          <select
            className="course-form-field__input"
          ...
          >
            <option value="">
              {lookupsLoading ? "กำลังโหลดคณะ..." : "เลือกคณะ"}
            </option>
      ...
      <div className="course-form-row">
        <div className="course-form-field">
          <div className="course-form-field__label-row">
            <label className="course-form-field__label">
              สาขา<span className="course-form-field__required">*</span>
            </label>
            <button
              type="button"
              className="course-form-field__link"
              onClick={onAddMajor}
            >
              <Plus size={13} />
              เพิ่มสาขาใหม่
          ...
          <select
            ...
            onChange={(e) =>
              updateForm((p) => ({
                ...p,
                majorId: parseInt(e.target.value, 10) || "",
              }))
            }
            disabled={lookupsLoading || !form.facultyId}
          >
            <option value="">
              {lookupsLoading ? "กำลังโหลดสาขา..." : "เลือกสาขา"}
            </option>
        ...
        <div className="course-form-field">
          <label className="course-form-field__label">
            ปีการศึกษา<span className="course-form-field__required">*</span>
          </label>
          <input
            type="number"
            className="course-form-field__input"
...
function StructureGuide({
  ...
  return (
    <section
      ...
      aria-labelledby="structure-guide-title"
      aria-live="polite"
    >
        ...
        {steps.map((item, index) => (
          ...
          >
            ...
            <span className="structure-guide__step-copy">
              <span className="structure-guide__step-label">
                {index + 1}. {item.label}
              </span>
...
function Step2({
  ...
  const collectCategoryIds = (cat) => [
    cat.id,
    ...(cat.children || []).flatMap(collectCategoryIds),
  ];
  ...
  const insertCategoryUnder = (cats, targetId, movedCategory) =>
    cats.map((cat) => {
      if (cat.id === targetId) {
    ...
    const updateChildren = (children) =>
      (children || []).map((child) => {
        ...
        const childCode =
          ...
          currentChildCode.startsWith(`${normalizedPreviousCode}.`)
  ...
  const categoryContains = (cat, targetId) =>
    (cat.children || []).some(
  ...
  const getCategorySubtreeDepth = (cat) => {
    ...
    return Math.max(
      ...cat.children.map((child) => 1 + getCategorySubtreeDepth(child)),
    );
  };
  ...
  const getDeleteCategoryPreview = (cat) => {
    ...
    const affectedCourses = categoryIds.flatMap((categoryId) =>
      (coursesByCategory[categoryId] || []).map((course) => ({
        ...course,
  ...
  const getDuplicateCourseIssues = (nextCourse) => {
    const otherCourses = getAllCurrentCourses().filter(
  ...
  const getAllCoursesInCategory = (cat) => {
    ...
    const childCourses = (cat.children || []).flatMap((c) =>
      getAllCoursesInCategory(c),
    );
    return [...courses, ...childCourses];
  };

  const courses = selectedCategory
    ? getAllCoursesInCategory(selectedCategory)
    : [];
  ...
  const getNextChildCode = (parentCode, siblings) =>
    `${parentCode}.${getNextCode(siblings)}`;
    ...
    setSelectedCategory((p) =>
      p?.id === id
        ? {
            ...p,
            name: uniqueName,
            code: String(nextCode ?? p.code ?? "").trim(),
          }
        : p,
    );
  };
  ...
  const handleApplyStructureImport = (rows) => {
    const preview = previewDraftCurriculumStructureImport(
      categories,
      coursesByCategory,
      rows,
    );
    const imported = applyDraftCurriculumStructureImport(
      categories,
      coursesByCategory,
      preview,
    );
  ...
  const confirmDeleteCategory = () => {
    ...
    const newCourses = Object.fromEntries(
      Object.entries(coursesByCategory).filter(
    ...
    const nextSelectedCategory = preview?.moveTarget
      ? findCategoryInfo(nextCategories, preview.moveTarget.id)?.category ||
        preview.moveTarget
      : null;
    ...
    if (selectedCategory) {
      if (categoryIds.includes(selectedCategory.id)) {
        ...
        setSelectedCategory(
          findCategoryInfo(nextCategories, selectedCategory.id)?.category ||
            selectedCategory,
        );
      }
    }
  ...
  const handleAddCourse = (courseData) => {
    ...
    setForm((p) => ({
      ...
      coursesByCategory: {
        ...p.coursesByCategory,
        [selectedCategory.id]: [
          ...(p.coursesByCategory[selectedCategory.id] || []),
          course,
        ],
      },
    }));
  ...
  const handleUpdateCourse = (updatedCourse) => {
    const ownerCategoryId =
      updatedCourse.ownerCategoryId || findCourseOwnerId(updatedCourse.id);
    ...
    setForm((p) => ({
      ...
      coursesByCategory: {
        ...p.coursesByCategory,
        [ownerCategoryId]: (p.coursesByCategory[ownerCategoryId] || []).map(
          (course) =>
            course.id === updatedCourse.id
              ? stripCourseMeta(updatedCourse)
              : course,
        ),
      },
    }));
  ...
  const handleCategoryDragOver = (event, targetCat) => {
    ...
    if (
      draggedCat.id === targetCat.id ||
      categoryContains(draggedCat, targetCat.id)
    ) {
      event.stopPropagation();
      setDropTargetCategoryId((current) =>
        current === targetCat.id ? null : current,
      );
      return;
    }
    ...
    const nextDepth =
      (targetInfo?.depth ?? 0) + 1 + getCategorySubtreeDepth(draggedCat);
  ...
  const handleCategoryDrop = (event, targetCat) => {
    ...
    const nextCategories = insertCategoryUnder(
      result.categories,
      targetCat.id,
      result.removed,
    );
    const nextSelectedCategory =
      findCategoryInfo(nextCategories, result.removed.id)?.category ||
      result.removed;
  ...
  const handleCategoryRootDragOver = (event) => {
    ...
    if (getCategorySubtreeDepth(draggedInfo.category) > MAX_CATEGORY_DEPTH)
      return;
  ...
  const handleCourseDrop = (event, targetCourse) => {
    ...
    const targetCategoryId =
      targetCourse.ownerCategoryId || findCourseOwnerId(targetCourse.id);
    setForm((p) => ({
      ...p,
      coursesByCategory: moveCourseInMap(
        p.coursesByCategory || {},
        draggedCourseId,
        targetCategoryId,
        targetCourse.id,
  ...
  const handleMoveCourseInEditor = ({
    draggedCourseId: movingCourseId,
    targetCategory,
    beforeCourseId = null,
  }) => {
    if (!movingCourseId || !targetCategory || targetCategory.children?.length)
      return;
    setForm((p) => ({
      ...p,
      coursesByCategory: moveCourseInMap(
  ...
  const addCredits = (cat, visited = new Set()) => {
    ...
    visited.add(cat.id);
    const ownCourseCredits = (coursesByCategory[cat.id] || []).reduce(
    ...
    return (
      ownCourseCredits +
      (cat.children || []).reduce(
        (sum, child) => sum + addCredits(child, visited),
        0,
      )
    );
  };
  ...
  const totalCredits = categories.reduce(
    (sum, c) => sum + getCategoryTotalCredits(c),
    0,
  );
  const totalCourses = Object.values(coursesByCategory).flat().length;
  const totalCategories = countCategories(categories);
  const deleteCategoryPreview = categoryToDelete
    ? getDeleteCategoryPreview(categoryToDelete)
    : null;
    ...
    <div className="course-structure-panel">
      ...
      <div className="course-stats-bar">
        <div className="course-stats-bar__item">
          ...
          <span className="course-stats-bar__text">
            <strong>{totalCategories}</strong> หมวด
          </span>
        </div>
        <div className="course-stats-bar__item">
          <BookOpen size={16} className="course-stats-bar__icon" />
          <span className="course-stats-bar__text">
            <strong>{totalCourses}</strong> วิชา
          </span>
        </div>
        <div className="course-stats-bar__item">
          <Award size={16} className="course-stats-bar__icon" />
          <span className="course-stats-bar__text">
            <strong>{totalCredits}</strong> หน่วยกิต
          </span>
        </div>
      </div>
      ...
      <div className="course-two-panel">
        ...
        <div className="course-two-panel__sidebar">
          <CurriculumStructureSidebar
            ...
            title={
              language === "en" ? "Category structure" : "โครงสร้างหมวดวิชา"
            }
        ...
        <CurriculumCourseEditorPanel
          ...
          categoryTotalCredits={
            selectedCategory ? getCategoryTotalCredits(selectedCategory) : 0
          }
          canEdit
          ...
          onRenameCategory={(id, updates) => {
            const changesIdentity =
              Object.prototype.hasOwnProperty.call(updates, "nameTh") ||
      ...
      <ConfirmActionModal
        ...
        onConfirm={() =>
          handleDeleteSelectedCourses(courseDeleteConfirmation?.courses || [])
        }
      />
      ...
      <ConfirmActionModal
        ...
        message={
          categoryToDelete
            ? `คุณแน่ใจหรือไม่ที่จะลบหมวดวิชา "${categoryToDelete.code} ${categoryToDelete.name || "ยังไม่ตั้งชื่อ"}"?`
            : ""
        }
        impact={
          deleteCategoryPreview && (
            <>
              <div>
                หมวดย่อยที่ได้รับผลกระทบ:{" "}
                {deleteCategoryPreview.affectedCategories.length} หมวด
              </div>
              <div>
                รายวิชาที่ได้รับผลกระทบ:{" "}
                {deleteCategoryPreview.affectedCourses.length} วิชา
              </div>
              {deleteCategoryPreview.moveTarget ? (
                <div>
                  รายวิชาจะถูกย้ายไปที่ "{deleteCategoryPreview.moveTarget.code}{" "}
                  {deleteCategoryPreview.moveTarget.name || "ยังไม่ตั้งชื่อ"}"
                </div>
              ) : (
                <div>
                  ไม่มีหมวดปลายทาง รายวิชาจะถูกถอดออกจากโครงสร้างปัจจุบัน
                </div>
              )}
            </>
          )
...
function Step3({ form, generatedCode }) {
  ...
  const getCategoryTotalCredits = (cat) =>
    getAllCoursesInCategory(cat).reduce(
      (sum, course) => sum + (Number(course.credits) || 0),
      0,
    );
  const totalCredits = categories.reduce(
  ...
  const totalCourses = Object.values(coursesByCategory).flat().length;
  const totalCategories = countCategories(categories);
  const coreCourses = Object.values(coursesByCategory)
    .flat()
    .filter((c) => c.isCoreCourse).length;
      ...
      <div key={cat.id} className="course-overview-tree-node">
        ...
        >
          <div className="course-overview-tree-category__main">
            <span className="course-overview-tree-category__code">
              {cat.code}
            </span>
            <span className="course-overview-tree-category__name">
              {cat.name || <em>ยังไม่ตั้งชื่อ</em>}
            </span>
          </div>
          ...
          >
            {directCourses.map((course) => (
              <div key={course.id} className="course-overview-tree-course">
                <span className="course-overview-tree-course__code">
                  {course.code || "-"}
                </span>
                <span className="course-overview-tree-course__name">
                  {course.nameTh || course.nameEn || "ยังไม่มีชื่อวิชา"}
                </span>
                <span className="course-overview-tree-course__credits">
                  {Number(course.credits) || 0} หน่วยกิต
                </span>
              </div>
            ))}
          </div>
        ...
        {(cat.children || []).map((child) =>
          renderCategoryOverview(child, depth + 1),
        )}
      </div>
    );
  };
    ...
    <div className="course-overview">
      ...
      <div className="course-overview-info">
        ...
        <div className="course-overview-info__grid">
          <div>
            <span className="course-overview-info__label">
              ชื่อหลักสูตร (ไทย):
            </span>{" "}
            <span className="course-overview-info__value">
              {form.nameTh || "-"}
            </span>
          </div>
          <div>
          ...
          <div>
            <span className="course-overview-info__label">รหัสหลักสูตร:</span>{" "}
            <span className="course-overview-info__value">
              {generatedCode || "-"}
            </span>
          </div>
          <div>
            <span className="course-overview-info__label">ปีการศึกษา:</span>{" "}
            <span className="course-overview-info__value">
              {form.year ? `ปีการศึกษา ${form.year}` : "-"}
            </span>
          </div>
        </div>
      </div>
...
function CreateCoursePageContent() {
  ...
  const duplicateNameWarning = buildDuplicateCurriculumNameWarning(
    form,
    majors,
    curriculums,
    language,
    ...
    async function loadLookups() {
      ...
      try {
        ...
        if (nextFaculties.length === 1) {
          ...
          const scopedMajors = nextMajors.filter(
  ...
  const countAllCategories = (categories = []) =>
    categories.reduce(
      (sum, category) => sum + 1 + countAllCategories(category.children || []),
      0,
    );

  const countAllCourses = (coursesByCategory = {}) =>
    Object.values(coursesByCategory).reduce(
  ...
  return (
    <div className="course-create-page">
      <div
        className={`course-create-container ${step === 2 ? "course-create-container--wide" : ""}`}
      >
        <div
          className={`course-form-container ${step === 2 ? "course-form-container--wide" : ""}`}
        >
          {/* Header */}
        ...
        <div className="course-form-nav">
          ...
          <div className="course-form-nav__right">
            ...
            {step === 3 && (
              ...
              >
                <Check size={15} />{" "}
                {submitting ? "กำลังบันทึก..." : "บันทึกหลักสูตร"}
              </button>
            )}
          </div>
        </div>
