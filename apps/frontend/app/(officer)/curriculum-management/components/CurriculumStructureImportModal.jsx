...
import { AlertTriangle, BookOpen, Download, FileSpreadsheet, FolderTree, Upload } from 'lucide-react';
...
const copy = {
    th: {
        ...
        nothingToImport: 'ไม่มีรายวิชาใหม่ให้นำเข้า กรุณาแก้ไขรหัสวิชาที่ซ้ำหรือเลือกไฟล์อื่น',
    ...
    en: {
        ...
        structurePreviewHint: 'Categories and courses from the file before they are added to the curriculum.',
        invalidStructurePreview: 'Structure preview with issues',
        invalidStructurePreviewHint: 'Items in red must be fixed before the data can be imported.',
        ...
        nothingToImport: 'There are no new courses to import. Update duplicate course codes or choose another file.',
...
const importIssueMessages = {
    th: {
        ...
        'category levels must be continuous from level 1': 'หมวดวิชาต้องระบุเรียงต่อเนื่องจากระดับ 1',
        'at least one category level is required': 'ต้องระบุหมวดวิชาอย่างน้อย 1 ระดับ',
        'category nesting cannot exceed 4 levels': 'หมวดวิชาซ้อนกันได้สูงสุด 4 ระดับ',
        'category code is required': 'ต้องระบุรหัสหมวดวิชา',
        'category code must contain up to 4 numeric levels': 'รหัสหมวดต้องเป็นตัวเลขคั่นด้วยจุด สูงสุด 4 ระดับ',
        'category name_th is required': 'ต้องระบุชื่อหมวดวิชาภาษาไทย',
        'category code must extend its parent code': 'รหัสหมวดย่อยต้องต่อจากรหัสหมวดแม่',
        'category code must match its category level': 'รหัสหมวดต้องตรงกับระดับหมวดวิชา',
        'category code conflicts with another imported category': 'รหัสหมวดขัดแย้งกับข้อมูลในไฟล์',
        'category code conflicts with the current curriculum structure': 'รหัสหมวดขัดแย้งกับโครงสร้างหลักสูตรเดิม',
        ...
        'รหัสหมวดต้องเป็นตัวเลขคั่นด้วยจุด สูงสุด 4 ระดับ': 'รหัสหมวดต้องเป็นตัวเลขคั่นด้วยจุด สูงสุด 4 ระดับ',
        ...
        'รหัสหมวดขัดแย้งกับโครงสร้างหลักสูตรเดิม': 'รหัสหมวดขัดแย้งกับโครงสร้างหลักสูตรเดิม',
    ...
    en: {
        ...
        'หมวดวิชาต้องระบุเรียงต่อเนื่องจากระดับ 1': 'Category levels must be continuous from level 1',
        'ต้องระบุหมวดวิชาอย่างน้อย 1 ระดับ': 'At least one category level is required',
        'รหัสหมวดต้องเป็นตัวเลขคั่นด้วยจุด สูงสุด 4 ระดับ': 'Category code must contain up to 4 numeric levels',
        'ต้องระบุชื่อหมวดวิชาภาษาไทย': 'Category Thai name is required',
        'รหัสหมวดย่อยต้องต่อจากรหัสหมวดแม่': 'Category code must extend its parent code',
        'รหัสหมวดต้องตรงกับระดับหมวดวิชา': 'Category code must match its category level',
        'รหัสหมวดขัดแย้งกับข้อมูลในไฟล์': 'Category code conflicts with another imported category',
        'รหัสหมวดขัดแย้งกับโครงสร้างหลักสูตรเดิม': 'Category code conflicts with the current curriculum structure',
...
function countPreviewCourses(category) {
    return (category.courses || []).length + (category.children || []).reduce((total, child) => total + countPreviewCourses(child), 0);
}

function sumPreviewCredits(category) {
    const directCredits = (category.courses || []).reduce((total, course) => total + (Number(course.credits) || 0), 0);
    return directCredits + (category.children || []).reduce((total, child) => total + sumPreviewCredits(child), 0);
}

function getPreviewIssues(issuesByRow, rowNumbers, fields) {
    const seen = new Set();
    return (rowNumbers || []).flatMap(rowNumber => issuesByRow.get(rowNumber) || [])
    ...
    return (
        <ul className="curriculum-import-preview__issue-list">
            {issues.map((issue, index) => (
                <li key={`${issue.field}-${index}`} className={issue.severity === 'warning' ? 'curriculum-import-preview__issue--warning' : ''}>
...
function ImportPreviewNode({ category, depth, labels, issuesByRow, language }) {
    ...
    const categoryIssues = getPreviewIssues(
        issuesByRow,
        category.rowNumbers,
        ['categories', `category_level_${depth + 1}`, `category_name_level_${depth + 1}`],
    );
    const categoryHasErrors = categoryIssues.some(issue => issue.severity !== 'warning');
    const categoryHasWarnings = categoryIssues.some(issue => issue.severity === 'warning');
    return (
        <li className="curriculum-import-preview__node" style={{ '--import-depth': depth }}>
            <div className={`curriculum-import-preview__category${categoryHasErrors ? ' curriculum-import-preview__category--invalid' : ''}${!categoryHasErrors && categoryHasWarnings ? ' curriculum-import-preview__category--warning' : ''}`}>
                <FolderTree size={15} aria-hidden="true" />
                <span className="curriculum-import-preview__category-name">
                    {category.code && <strong>{category.code}</strong>} {category.nameTh || '-'}
                </span>
                <span className="curriculum-import-preview__category-meta">
                    {courseCount} {labels.courseUnit} · {credits} {labels.creditUnit}
                </span>
                {categoryIssues.length > 0 && <AlertTriangle size={14} className={`curriculum-import-preview__warning-icon${!categoryHasErrors && categoryHasWarnings ? ' curriculum-import-preview__warning-icon--warning' : ''}`} aria-hidden="true" />}
            </div>
            ...
            {category.courses?.length > 0 && (
                <ul className="curriculum-import-preview__courses">
                    {category.courses.map((course, index) => {
                        ...
                        return (
                            <li key={`${course.code}-${index}`} className={`curriculum-import-preview__course${courseHasErrors ? ' curriculum-import-preview__course--invalid' : ''}${!courseHasErrors && courseHasWarnings ? ' curriculum-import-preview__course--warning' : ''}`}>
...
export default function CurriculumStructureImportModal({
    ...
    const importTree = useMemo(() => buildCurriculumStructureImportTree(rows), [rows]);
    ...
    const globalIssues = useMemo(() => issues.filter(issue => Number(issue?.row_number || 0) <= 0), [issues]);
    ...
    const handleFile = async event => {
        ...
        try {
            ...
            if (mode === 'draft') {
                ...
                setPreview({ valid: false, can_import: false, issues: parsed.issues, new_category_count: 0, existing_category_count: 0, course_count: 0, skipped_course_count: 0 });
            } else {
                ...
                {preview && (
                    <>
                        <div className="curriculum-import-modal__summary">
                            ...
                            <div><span>{labels.skippedCourses}</span><strong>{preview.skipped_course_count || 0}</strong></div>
                        </div>
                        {blockingIssues.length > 0 && (
                            <div className="curriculum-import-modal__issues">
                                <strong>{labels.issues}</strong>
                                <ul>{blockingIssues.map((entry, index) => <li key={`${entry.row_number}-${entry.field}-${index}`}>{issueLabel(entry, language)}</li>)}</ul>
                            </div>
                        )}
                        {warningIssues.length > 0 && (
                            <div className="curriculum-import-modal__warnings">
                                ...
                                <ul>{warningIssues.map((entry, index) => <li key={`${entry.row_number}-${entry.field}-${index}`}>{issueLabel(entry, language)}</li>)}</ul>
                            </div>
                        )}
                        {importTree.length > 0 && (
                            <section className={`curriculum-import-preview${preview.valid ? '' : ' curriculum-import-preview--invalid'}`} aria-label={labels.structurePreview}>
                                <div className="curriculum-import-preview__heading">
                                    <strong>{preview.valid ? labels.structurePreview : labels.invalidStructurePreview}</strong>
                                    <p>{preview.valid ? labels.structurePreviewHint : labels.invalidStructurePreviewHint}</p>
                                </div>
                                {globalIssues.length > 0 && (
                                    <div className="curriculum-import-preview__global-issues">
                                        ...
                                        <div>
                                            ...
                                            <PreviewIssueList issues={globalIssues} language={language} />
                                        </div>
                                    </div>
                                )}
                        ...
                        {preview.valid && !preview.can_import ? <p className="curriculum-import-modal__warning-text">{labels.nothingToImport}</p> : null}
