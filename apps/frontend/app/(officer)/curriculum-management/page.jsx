...
function findCategoryInfo(cats, id, parent = null, siblings = cats, depth = 0) {
  for (let index = 0; index < cats.length; index += 1) {
    ...
    const childInfo = findCategoryInfo(
      category.children || [],
      id,
      category,
      category.children || [],
...
function categoryContains(category, targetId) {
  return (category.children || []).some(
    (child) => child.id === targetId || categoryContains(child, targetId),
  );
}
...
function getCategorySubtreeDepth(category) {
  ...
  return Math.max(
    ...category.children.map((child) => 1 + getCategorySubtreeDepth(child)),
  );
}
...
function getCourseIdentity(course) {
  return String(
    course?.id ?? course?.curriculumCourseId ?? course?.courseId ?? "",
  );
}
...
function getDuplicateCourseIssues(nextCourse, allCourses) {
  ...
  const otherCourses = (allCourses || []).filter(
    (course) => getCourseIdentity(course) !== nextIdentity,
  );
  const issues = [];
  ...
  if (
    nextCode &&
    otherCourses.some(
      (course) => normalizeInsensitive(course.code) === nextCode,
    )
  ) {
  ...
  if (
    nextNameTh &&
    otherCourses.some((course) => normalizeName(course.nameTh) === nextNameTh)
  ) {
  ...
  if (
    nextNameEn &&
    otherCourses.some(
      (course) => normalizeInsensitive(course.nameEn) === nextNameEn,
    )
  ) {
...
function buildConfirmationMessage(error) {
  ...
  const templateList = templates
    ...
    .map(
      (template) =>
        `- ${template.name || template.code || "Template"}${template.cohort_year_be ? ` (${template.cohort_year_be})` : ""}`,
    )
    .join("\n");
  const more =
...
const BACKEND_MESSAGE_TH = {
  "curriculum structure is not ready to activate":
    "โครงสร้างหลักสูตรยังไม่พร้อมเปิดใช้งาน",
  "status must be draft, active, or inactive": "สถานะหลักสูตรไม่ถูกต้อง",
  "curriculum cannot transition back to draft":
    "หลักสูตรที่ออกจาก Draft แล้ว ไม่สามารถย้อนกลับเป็น Draft ได้",
  "draft curriculum can only transition to active":
    "หลักสูตร Draft สามารถเปลี่ยนเป็นพร้อมใช้งานได้เท่านั้น",
  "active curriculum can only transition to inactive":
    "หลักสูตรที่พร้อมใช้งานสามารถเปลี่ยนเป็นปิดใช้งานได้เท่านั้น",
  "inactive curriculum can only transition to active":
    "หลักสูตรที่ปิดใช้งานสามารถเปิดใช้งานได้เท่านั้น",
  "curriculum status is invalid": "สถานะหลักสูตรปัจจุบันไม่ถูกต้อง",
  "curriculum cannot be inactive while active templates are connected":
    "ยังปิดใช้งานหลักสูตรไม่ได้ เพราะมี Active Template เชื่อมอยู่",
  "curriculum cannot be deleted while templates are connected":
    "ไม่สามารถลบหลักสูตรนี้ได้ เพราะมี Template เชื่อมอยู่",
  "curriculum cannot be deleted because it has real usage":
    "ไม่สามารถลบหลักสูตรนี้ได้ เพราะมีข้อมูลการใช้งานจริงแล้ว ให้เปลี่ยนสถานะเป็นปิดใช้งานแทน",
  "curriculum structure is locked while active templates are connected":
    "โครงสร้างหลักสูตรถูกล็อกอยู่ เพราะมี Active Template เชื่อมอยู่",
  "curriculum metadata is locked while active templates are connected":
    "ข้อมูลหลักสูตรถูกล็อกอยู่ เพราะมี Active Template เชื่อมอยู่",
  "inactive curriculum edit requires confirmation because templates are connected":
    "หลักสูตรนี้มี Template เชื่อมอยู่ กรุณายืนยันก่อนแก้ไข",
  "inactive curriculum metadata edit requires confirmation because templates are connected":
    "หลักสูตรนี้มี Template เชื่อมอยู่ กรุณายืนยันก่อนแก้ไขข้อมูล",
  "curriculum major can only be changed while draft":
    "เปลี่ยนสาขาได้เฉพาะหลักสูตรที่อยู่ในสถานะ Draft",
  "major_id must be active when changing curriculum major":
    "สาขาปลายทางต้องเปิดใช้งานอยู่",
  "category delete requires confirmation":
    "การลบหมวดวิชาต้องได้รับการยืนยันก่อน",
  ...
  "category cannot be moved under itself or its child category":
    "ไม่สามารถย้ายหมวดวิชาไปอยู่ใต้ตัวเองหรือหมวดย่อยของตัวเองได้",
  "category name_th is required": "กรุณากรอกชื่อหมวดวิชาภาษาไทย",
  "category name_th cannot be empty": "ชื่อหมวดวิชาภาษาไทยห้ามว่าง",
  "category required_credits must be zero or greater":
    "หน่วยกิตของหมวดวิชาต้องไม่น้อยกว่า 0",
  "category display_order must be zero or greater":
    "ลำดับหมวดวิชาต้องไม่น้อยกว่า 0",
  ...
  "course display_order must be zero or greater":
    "ลำดับรายวิชาต้องไม่น้อยกว่า 0",
  ...
  "course_id is not allowed when creating curriculum courses":
    "การสร้างหลักสูตรต้องสร้างรายวิชาใหม่ ไม่สามารถอ้างอิง course_id เดิมได้",
  ...
  "curriculum code already exists in this major":
    "รหัสหลักสูตรนี้มีอยู่แล้วในสาขานี้",
  "curriculum name already exists in this major and effective year":
    "มีชื่อหลักสูตรนี้อยู่แล้วในสาขาและปีการศึกษานี้",
...
const CURRICULUM_VIOLATION_TH = {
  "curriculum must have at least one category":
    "ต้องมีหมวดวิชาอย่างน้อย 1 หมวด",
  "curriculum must have at least one course":
    "ต้องมีรายวิชาอย่างน้อย 1 รายวิชา",
  "all courses must have code, name_th, and credits >= 0":
    "รายวิชาทุกตัวต้องมีรหัสวิชา ชื่อวิชาภาษาไทย และหน่วยกิตต้องไม่ติดลบ",
  "course code must be unique within the curriculum":
    "รหัสวิชาต้องไม่ซ้ำกันภายในหลักสูตร",
};
...
function mapCurriculumError(error) {
  ...
  if (error?.code === "DUPLICATE") {
    return {
      userMessage:
        translateBackendMessage(englishMessage) || "ข้อมูลซ้ำกับรายการเดิม",
      debugMessage: englishMessage,
    };
  }
  if (violations.length > 0) {
    const violationText = violations
      .map((violation) => `- ${translateViolation(violation)}`)
      .join("\n");
    return {
  ...
  return {
    userMessage:
      translateBackendMessage(englishMessage) ||
      "ไม่สามารถบันทึกข้อมูลหลักสูตรได้",
    debugMessage: englishMessage,
  };
}
...
function CourseCard({ course, onDuplicate, onDelete, onOpen }) {
  return (
    <div className="course-card" onClick={() => onOpen(course)}>
      ...
      <h3 className="course-card__title">{course.nameTh}</h3>
...
function EmptyState({ onCreate }) {
  return (
    <div className="course-empty">
      ...
      <p className="course-empty__desc">
        เริ่มต้นสร้างหลักสูตรแรกของคุณเพื่อจัดการหลักสูตรนักศึกษา
      </p>
...
function DuplicateCurriculumModal({
  ...
  const hasSourceMajor = majors.some(
    (major) => String(major.majorId) === String(source.majorId),
  );
    ...
    <BaseModal
      ...
      footer={
        <>
          ...
          <button
            type="submit"
            form="duplicate-curriculum-form"
...
export default function CurriculumManagementPage() {
  ...
  const [metadataEditState, setMetadataEditState] = useState({
    editing: false,
    dirty: false,
  });
  const [metadataResetVersion, setMetadataResetVersion] = useState(0);
  const [metadataImpactConfirmation, setMetadataImpactConfirmation] =
    useState(null);
  ...
  const totalCurriculumCredits = courses.reduce(
    (sum, c) => sum + (c.stats?.totalCredits || 0),
    0,
  );
  ...
  const handleOpenCourse = useCallback(async (course) => {
    ...
    setSuccess("");
    try {
      const detail = await fetchCurriculumDetail(
  ...
  const handleConfirmDuplicateCourse = useCallback(
    async (event) => {
      ...
      setSuccess("");
      try {
        await duplicateCurriculum(
          duplicatingCourse.curriculumId || duplicatingCourse.id,
          duplicateForm,
        );
  ...
  const handleDeleteCourse = useCallback((course) => {
    if (course.templateCount > 0) {
      alert(
        `ไม่สามารถลบหลักสูตร "${course.nameTh}" ได้ เนื่องจากมี Template เชื่อมอยู่ ${course.templateCount} รายการ`,
      );
      return;
    }
    setError("");
  ...
  const [courseDeleteConfirmation, setCourseDeleteConfirmation] =
    useState(null);
  const [courseDeleteImpactConfirmation, setCourseDeleteImpactConfirmation] =
    useState(null);
  ...
  useEffect(() => {
    ...
    setSelectedCategory((current) =>
      current ? findById(nextCategories, current.id) : null,
    );
    if (!selectedCourse) {
      setShowAllCourses(false);
    }
  ...
  const commitCurriculumMutation = useCallback(
    async (operation, successMessage) => {
      ...
      for (;;) {
        ...
        try {
          ...
          if (
            needsConfirmation &&
            window.confirm(buildConfirmationMessage(err))
          ) {
            confirmImpact = true;
            continue;
          }
  ...
  const handleCommitStructureImport = useCallback(
    async (rows, confirmImpact) => {
      if (!selectedCourse) return null;
      setOperationLoading(true);
      setError("");
      setErrorDebug("");
      try {
        const detail = await commitCurriculumStructureImport(
  ...
  const handleAddCategory = useCallback(
    async (parentCategory = null) => {
      ...
      await commitCurriculumMutation(
        (confirmImpact) =>
          createCurriculumCategory(
            selectedCourse.curriculumId,
            {
  ...
  const handleRenameCategory = useCallback(
    async (id, updates) => {
      ...
      const payload =
        typeof updates === "string" ? { nameTh: updates } : updates;
      return commitCurriculumMutation(
        (confirmImpact) =>
          updateCurriculumCategory(
            selectedCourse.curriculumId,
            id,
            {
              ...
              requiredCredits:
                payload.requiredCredits ?? current.requiredCredits,
  ...
  const handleRequestDeleteCategory = useCallback(
    async (cat) => {
      ...
      setErrorDebug("");
      try {
        const preview = await getDeleteCurriculumCategoryPreview(
          selectedCourse.curriculumId,
          cat.id,
        );
  ...
  const handleAddCourse = useCallback(
    async (data) => {
      ...
      const displayOrder =
        (coursesByCategory[selectedCategory.id] || []).length + 1;
      return commitCurriculumMutation(
        (confirmImpact) =>
          createCurriculumCourse(
            selectedCourse.curriculumId,
  ...
  const handleValidateCourseBeforeSave = useCallback(
  ...
  const handleRequestDeleteCourseInEditor = useCallback(
    (course) => {
  ...
  const handleRequestBulkDeleteCoursesInEditor = useCallback(
    (selectedCourses) => {
  ...
  const handleConfirmDeleteCourseInEditor = useCallback(
    async (course, confirmImpact = false) => {
      if (!selectedCourse || !course?.curriculumCourseId) return false;
      setOperationLoading(true);
      setError("");
      setErrorDebug("");
  ...
  const handleConfirmBulkDeleteCoursesInEditor = useCallback(
    async (selectedCourses, confirmImpact = false, resumeIndex = 0) => {
      if (!selectedCourse || !selectedCourses?.length) return false;
      let lastDetail = null;
      setOperationLoading(true);
      setError("");
  ...
  const handleCategoryDragOver = useCallback(
    (event, targetCategory) => {
      const draggedInfo = draggedCategoryId
        ? findCategoryInfo(categories, draggedCategoryId)
        : null;
      ...
      if (
        ...
        categoryContains(draggedCategory, targetCategory.id)
      ) {
        event.stopPropagation();
        setDropTargetCategoryId((current) =>
          current === targetCategory.id ? null : current,
        );
        return;
      }
      ...
      const nextDepth =
        (targetInfo?.depth ?? 0) + 1 + getCategorySubtreeDepth(draggedCategory);
      if (nextDepth > 3) {
        event.stopPropagation();
        setDropTargetCategoryId((current) =>
          current === targetCategory.id ? null : current,
  ...
  const handleCategoryRootDrop = useCallback(
    async (event) => {
      ...
      const displayOrder =
        categories.filter((category) => category.id !== draggedCategoryId)
          .length + 1;
      await handleRenameCategory(draggedCategoryId, {
        parentId: null,
        displayOrder,
  ...
  const handleCourseCategoryDragOver = useCallback(
    (event, targetCategory) => {
  ...
  const handleCourseCategoryDrop = useCallback(
    async (event, targetCategory) => {
      if (
        !selectedCourse ||
        !draggedCourseId ||
      ...
      const sourceCategoryId = findCourseOwnerId(
        draggedCourseId,
        coursesByCategory,
      );
      const movingCourse = sourceCategoryId
        ? (coursesByCategory[sourceCategoryId] || []).find(
      ...
      const displayOrder =
        (coursesByCategory[targetCategory.id] || []).length + 1;
      await commitCurriculumMutation(
        (confirmImpact) =>
          updateCurriculumCoursePlacement(
            selectedCourse.curriculumId,
            movingCourse.curriculumCourseId,
            {
  ...
  const handleMoveCourseInEditor = useCallback(
    async ({
      draggedCourseId: movingCourseId,
      targetCategory,
      beforeCourseId = null,
    }) => {
      if (
      ...
      const movingCourse = sourceCategoryId
        ? (coursesByCategory[sourceCategoryId] || []).find(
            (course) => String(course.id) === String(movingCourseId),
          )
        : null;
      ...
      const targetIndex = beforeCourseId
        ? targetList.findIndex(
            (course) => String(course.id) === String(beforeCourseId),
          )
        : -1;
      const displayOrder =
        targetIndex >= 0 ? targetIndex + 1 : targetList.length + 1;

      const detail = await commitCurriculumMutation(
        (confirmImpact) =>
          updateCurriculumCoursePlacement(
            selectedCourse.curriculumId,
            movingCourse.curriculumCourseId,
  ...
  const handleSaveMetadata = useCallback(
    async (form, confirmImpact = false) => {
      ...
      try {
        const detail = await updateCurriculumMetadata(
          selectedCourse.curriculumId,
          form,
          confirmImpact,
        );
        ...
        if (
          ...
          setMetadataFieldErrors({
            code: "รหัสหลักสูตรนี้มีอยู่แล้วในสาขานี้",
          });
          return false;
        }
        if (
          ...
          setMetadataFieldErrors({
            nameTh: "มีชื่อหลักสูตรนี้อยู่แล้วในสาขาและปีการศึกษาที่เลือก",
          });
          return false;
        }
  ...
  const handleRequestChangeStatus = useCallback(
    (nextStatus) => {
      if (metadataEditState.dirty) {
  ...
  const statusAction = selectedCourse
    ? getStatusAction(selectedCourse.status)
    : null;
    ...
    <>
      ...
      {view === "list" ? (
        <div className="course-list-page">
          ...
          <div className="course-list-header">
            <div className="course-list-header__left">
              ...
              <p className="course-list-header__subtitle">
                สร้างและจัดการหลักสูตรสำหรับนักศึกษา
              </p>
            </div>
          ...
          <div className="course-stats-row">
            ...
            <div className="course-stat-card">
              ...
              <div className="course-stat-card__content">
                <span className="course-stat-card__value">
                  {totalTemplates}
                </span>
        ...
        <div className="course-editor-page">
          ...
          <div className="course-editor-header">
            <div className="course-editor-header__left">
              ...
              <div className="course-editor-header__title">
                ...
                <p className="course-editor-header__subtitle">
                  {selectedCourse.nameEn ? `${selectedCourse.nameEn} · ` : ""}ปี{" "}
                  {selectedCourse.year}
                </p>
              </div>
            </div>
            <div className="course-editor-header__actions">
              <span
                className={`course-status-badge course-status-badge--${selectedCourse.status}`}
              >
              ...
              {statusAction && (
                <button
                  ...
                  onClick={() =>
                    handleRequestChangeStatus(statusAction.nextStatus)
                  }
          ...
          <div
            className="curriculum-detail-tabs"
            role="tablist"
              ...
              <div className="course-editor-panels">
                <div className="course-editor-panels__sidebar">
                  <CurriculumStructureSidebar
                    ...
                    addDisabled={
                      selectedCategory
                        ? getCategoryDepth(selectedCategory, categories) >= 3
                        : false
                    }
                    clearSelectionDisabled={
                      operationLoading || isCourseEditorEditing
                    }
                    ...
                    headerActions={
                      ...
                      >
                        <Upload size={12} />{" "}
                        {language === "en" ? "Import courses" : "นำเข้ารายวิชา"}
                      </button>
                    }
                ...
                <div className="course-editor-panels__content">
                  <CurriculumCourseEditorPanel
                    ...
                    category={
                      showAllCourses
                        ? { id: "all", code: "", name: "วิชาทั้งหมดในหลักสูตร" }
                        : selectedCategory
                    }
                    courses={getDisplayCourses(
                      categories,
                      selectedCategory,
                      showAllCourses,
                    )}
                    ...
                    categoryTotalCredits={
                      showAllCourses
                        ? (selectedCourse.stats?.totalCredits ?? 0)
                        : getDisplayCourses(
                            categories,
                            selectedCategory,
                            false,
                    ...
                    draggedCourseId={draggedCourseId}
                    onRenameCategory={
                      selectedCategory ? handleRenameCategory : () => {}
                    }
                    allowCategoryCodeEdit
                    onDeleteCategory={
                      selectedCategory ? handleRequestDeleteCategory : () => {}
                    }
      ...
      <ConfirmActionModal
        ...
        title={
          courseDeleteConfirmation?.mode === "bulk"
      ...
      <ConfirmActionModal
        ...
        message={
          courseDeleteImpactConfirmation?.error
            ? buildConfirmationMessage(courseDeleteImpactConfirmation.error)
            : ""
        }
        ...
        onCancel={() =>
          !operationLoading && setCourseDeleteImpactConfirmation(null)
        }
      />
      ...
      <ConfirmActionModal
        ...
        message={
          <>
            ยืนยันการลบหลักสูตร <strong>{deletingCourse?.nameTh}</strong>{" "}
            หรือไม่?
          </>
        }
      ...
      <ConfirmActionModal
        ...
        message={
          <>
            ...
            <strong>
              {deletingCategory
                ? `${deletingCategory.code} ${deletingCategory.name || ""}`
                : ""}
            </strong>{" "}
            หรือไม่?
          </>
      ...
      <ConfirmActionModal
        ...
        impact={
          <ul className="curriculum-metadata-impact-list">
            {(metadataImpactConfirmation?.affectedTemplates || []).map(
              (template) => (
                <li
                  key={template.curriculum_template_id || template.template_id}
                  className={`curriculum-metadata-impact-list__item curriculum-metadata-impact-list__item--${template.severity || "warning"}`}
                >
                  <strong>
                    {template.name || template.code || "Template"}
                  </strong>
                  {template.cohort_year_be ? (
                    <span>รุ่น {template.cohort_year_be}</span>
                  ) : null}
                  <span>
        ...
        loading={operationLoading}
        onConfirm={() =>
          metadataImpactConfirmation &&
          handleSaveMetadata(metadataImpactConfirmation.form, true)
        }
        onCancel={() =>
          !operationLoading && setMetadataImpactConfirmation(null)
        }
      />
