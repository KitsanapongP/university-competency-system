...
function gradingModeLabel(value, language) {
    return GRADING_MODE_LABELS[language]?.[value] || GRADING_MODE_LABELS.en[value] || value;
}
...
const BACKEND_SESSION_MESSAGE_TH = {
    ...
    'closed or cancelled activity sessions are read-only': 'กิจกรรมที่ปิดหรือยกเลิกแล้วไม่สามารถแก้ไขรอบกิจกรรมได้',
    'finalized session cannot be changed': 'รอบกิจกรรมนี้ถูก Finalize แล้ว ไม่สามารถแก้ไขได้',
    'session is already finalized': 'รอบกิจกรรมนี้ถูก Finalize แล้ว',
    'cancelled session cannot be changed': 'รอบกิจกรรมที่ยกเลิกแล้วไม่สามารถแก้ไขได้',
    'cancelled session cannot be finalized': 'รอบกิจกรรมที่ยกเลิกแล้วไม่สามารถ Finalize ได้',
    'session status can only change from scheduled': 'เปลี่ยนสถานะได้เฉพาะรอบกิจกรรมที่ยัง Scheduled',
    'invalid session status transition': 'ไม่สามารถเปลี่ยนสถานะรอบกิจกรรมตามลำดับนี้ได้',
    ...
    'late_at must be after start_at': 'กำหนดเวลาสายต้องไม่เร็วกว่ารอบกิจกรรมเริ่ม',
    'late_at must be before end_at': 'กำหนดเวลาสายต้องอยู่ก่อนเวลาสิ้นสุดรอบกิจกรรม',
    ...
    'assignee must be active staff in activity faculty': 'เลือกได้เฉพาะ staff ที่ยังใช้งานอยู่ในคณะของกิจกรรมนี้',
    ...
    'session needs at least one assignee before finalize': 'ต้องมีผู้รับผิดชอบอย่างน้อย 1 คนก่อน Finalize',
    'session needs at least one competency before finalize': 'ต้องมี Competency อย่างน้อย 1 รายการก่อน Finalize',
    'session competency percent total must equal 100': 'สัดส่วน Competency รวมต้องเท่ากับ 100% ก่อน Finalize',
    'finalized session cannot be deleted': 'ไม่สามารถลบรอบกิจกรรมที่ Finalize แล้ว',
    'session cannot be deleted because registration or attendance exists': 'รอบกิจกรรมนี้มีข้อมูลลงทะเบียนหรือเช็กชื่อแล้ว ให้ยกเลิกแทนการลบ',
...
function mapSessionError(error) {
    ...
    return BACKEND_SESSION_MESSAGE_TH[message] || message || 'ไม่สามารถดำเนินการกับรอบกิจกรรมได้';
}
...
function StatusBadge({ status, finalized }) {
    return (
        <span className={`activity-session-status activity-session-status--${status || 'scheduled'}`}>
...
function SessionForm({ form, setForm, disabled }) {
    ...
    const scoringModeLabel = language === 'en' ? 'Scoring method' : 'รูปแบบการให้คะแนน';

    return (
        <div className="activity-session-form">
            <div className="course-row">
                <label className="course-field">
                    <span className="course-label">วันที่เริ่มต้น<span className="course-required">*</span></span>
                    <input
                        type="date"
                        className="course-input"
                        value={datePart(form.startAt)}
                        onChange={event => updateDateTimePart(setForm, 'startAt', 'date', event.target.value)}
                        disabled={disabled}
                    />
                </label>
                <label className="course-field">
                    <span className="course-label">เวลาเริ่มต้น<span className="course-required">*</span></span>
                    <input
                        type="time"
                        className="course-input"
                        value={timePart(form.startAt)}
                        onChange={event => updateDateTimePart(setForm, 'startAt', 'time', event.target.value)}
                        disabled={disabled}
                    />
                </label>
                <label className="course-field">
                    <span className="course-label">วันที่สิ้นสุด<span className="course-required">*</span></span>
                    <input
                        type="date"
                        className="course-input"
                        value={datePart(form.endAt)}
                        onChange={event => updateDateTimePart(setForm, 'endAt', 'date', event.target.value)}
                        disabled={disabled}
                    />
                </label>
                <label className="course-field">
                    <span className="course-label">เวลาสิ้นสุด<span className="course-required">*</span></span>
                    <input
                        type="time"
                        className="course-input"
                        value={timePart(form.endAt)}
                        onChange={event => updateDateTimePart(setForm, 'endAt', 'time', event.target.value)}
                        disabled={disabled}
                    />
                </label>
            </div>
            ...
            <label className="course-field">
                ...
                <textarea
                    ...
                    onChange={event => setForm(prev => ({ ...prev, locationDetail: event.target.value }))}
            ...
            <div className="course-row">
                <label className="course-field">
                    ...
                    <select
                        ...
                        onChange={event => setForm(prev => ({ ...prev, gradingMode: event.target.value }))}
                        disabled={disabled}
                    >
                ...
                <label className="course-field">
                    ...
                    <input
                        ...
                        onChange={event => setForm(prev => ({ ...prev, maxRawScore: event.target.value }))}
                        disabled={disabled}
                    />
                </label>
                <label className="course-field">
                    ...
                    <input
                        ...
                        onChange={event => setForm(prev => ({ ...prev, passThreshold: event.target.value }))}
            ...
            <div className="course-row">
                <label className="course-field">
                    ...
                    <input
                        ...
                        onChange={event => setForm(prev => ({ ...prev, lateAtTime: event.target.value }))}
                ...
                <label className="course-field">
                    ...
                    <input
                        ...
                        onChange={event => setForm(prev => ({ ...prev, latePenaltyFactor: event.target.value }))}
                        disabled={disabled}
                    />
                </label>
                <label className="course-field">
                    ...
                    <input
                        ...
                        onChange={event => setForm(prev => ({ ...prev, minAttendanceMinutes: event.target.value }))}
                        placeholder="ไม่กำหนด"
                        disabled={disabled}
                    />
                </label>
            ...
            <div className="activity-session-check-row">
                ...
                <label className="activity-management__checkbox">
                    <input
                        ...
                        onChange={event => setForm(prev => ({ ...prev, requireCheckout: event.target.checked }))}
                        disabled={disabled}
                    />
...
export default function ActivitySessionManagementPage() {
    ...
    const activityReadOnly = activity && !['draft', 'published'].includes(activity.status);
    const sessionReadOnly = activityReadOnly || !selectedSession?.canEdit;
    const competencyTotal = useMemo(
        () => competencyRows.reduce((sum, item) => sum + (Number(item.maxPercent) || 0), 0),
        [competencyRows],
    );
    const canFinalizeSelected = selectedSession?.canFinalize
    ...
    const loadList = useCallback(async ({ keepSelection = true } = {}) => {
        ...
        } else if (!nextSessions.some(item => item.sessionId === selectedSessionId)) {
            setSelectedSessionId(nextSessions[0]?.sessionId || null);
        }
        return nextSessions;
    ...
    const loadPage = useCallback(async () => {
        ...
        try {
            const [nextActivity, nextSessions, nextAssignees, nextCompetencies] = await Promise.all([
    ...
    const handleCreateSession = async () => {
        ...
        try {
            ...
            setSelectedSessionId(created.sessionId || nextSessions[0]?.sessionId || null);
        } catch (err) {
            setFeedback({ type: 'error', message: mapSessionError(err) });
        } finally {
            setSubmitting(false);
    ...
    const handleSaveInfo = async () => {
        ...
        setFeedback({ type: '', message: '' });
        try {
            const updated = await updateActivitySession(selectedSession.sessionId, sessionForm);
    ...
    const handleSaveAssignments = async () => {
        ...
        setFeedback({ type: '', message: '' });
        try {
            await replaceSessionAssignments(selectedSession.sessionId, assignmentRows);
    ...
    const handleSaveCompetencies = async () => {
        ...
        setFeedback({ type: '', message: '' });
        try {
            await replaceSessionCompetencies(selectedSession.sessionId, competencyRows);
            setFeedback({ type: 'success', message: 'บันทึก Competency mapping แล้ว' });
            await refreshSelectedAndList();
        } catch (err) {
            setFeedback({ type: 'error', message: mapSessionError(err) });
        } finally {
    ...
    const handleStatusChange = async (session, status) => {
        ...
        setFeedback({ type: '', message: '' });
        try {
            const updated = await updateActivitySessionStatus(session.sessionId, status);
            setFeedback({ type: 'success', message: status === 'completed' ? 'เปลี่ยนสถานะเป็น Completed แล้ว' : 'ยกเลิกรอบกิจกรรมแล้ว' });
            setSelectedSession(updated);
    ...
    const selectedSummary = selectedSession
        ? sessions.find(item => item.sessionId === selectedSession.sessionId) || selectedSession
        : null;
    ...
    const confirmConfig = (() => {
        ...
        if (confirmAction.type === 'complete') {
            return {
                ...
                message: <>ยืนยันว่ารอบกิจกรรม <strong>ครั้งที่ {confirmAction.session.sessionNo}</strong> ดำเนินการเสร็จแล้ว?</>,
        ...
        if (confirmAction.type === 'cancel') {
            return {
                ...
                message: <>ยืนยันการยกเลิกรอบกิจกรรม <strong>ครั้งที่ {confirmAction.session.sessionNo}</strong> หรือไม่?</>,
        ...
        if (confirmAction.type === 'finalize') {
            return {
                ...
                message: <>ยืนยันการ lock setup ของรอบกิจกรรม <strong>ครั้งที่ {confirmAction.session.sessionNo}</strong> หรือไม่?</>,
        ...
        return {
            title: 'ลบรอบกิจกรรม',
            message: <>ยืนยันการลบรอบกิจกรรม <strong>ครั้งที่ {confirmAction.session.sessionNo}</strong> หรือไม่?</>,
    ...
    return (
        <div className="activity-session-page">
            <div className="course-list-header">
                <div className="course-list-header__left">
                    ...
                    <p className="course-list-header__subtitle">
                        {activity ? `${activity.code} - ${activity.nameTh}` : 'กำลังโหลดข้อมูลกิจกรรม...'}
                    </p>
                </div>
                <div className="course-list-header__actions">
                    <button className="course-btn course-btn--ghost" onClick={loadPage} disabled={loading || submitting}>
                        <RefreshCw size={16} className={loading ? 'activity-management__spin' : ''} />
                        โหลดใหม่
                    </button>
                    <button className="course-btn course-btn--primary" onClick={handleOpenCreate} disabled={loading || submitting || activityReadOnly}>
                        <Plus size={16} />
                        เพิ่มรอบกิจกรรม
                    </button>
                </div>
            ...
            <ToastNotifications
                ...
                onCloseSuccess={() => setFeedback(current => (current.type === 'success' ? { type: '', message: '' } : current))}
                onCloseError={() => setFeedback(current => (current.type === 'error' ? { type: '', message: '' } : current))}
            />
            ...
            <section className="activity-session-shell">
                <aside className="activity-session-list">
                    ...
                    <div className="activity-session-list__items">
                        {loading ? (
                            <div className="activity-session-empty">กำลังโหลดรอบกิจกรรม...</div>
                        ) : sessions.length ? (
                            sessions.map(session => (
                                ...
                                >
                                    ...
                                    <div className="activity-session-card__meta">
                                        <span><Users size={13} /> {session.assignmentCount}</span>
                                        <span><Award size={13} /> {session.competencyCount}</span>
                ...
                <main className="activity-session-detail">
                    {!selectedSession ? (
                        <div className="activity-session-detail__empty">
                            ...
                            <span>Prototype รอบนี้ยังไม่ทำ registration, attendance และ scoring รายบุคคล</span>
                        </div>
                    ) : (
                        <>
                            <div className="activity-session-detail__header">
                                <div>
                                    <span className="activity-session-kicker">Session #{selectedSession.sessionNo}</span>
                                ...
                                <div className="activity-session-detail__actions">
                                    ...
                                    <button
                                        className="course-btn course-btn--ghost"
                                        onClick={() => setConfirmAction({ type: 'cancel', session: selectedSession })}
                                    ...
                                    <button
                                        ...
                                        onClick={() => setConfirmAction({ type: 'finalize', session: selectedSession })}
                                        disabled={!canFinalizeSelected || submitting}
                                        title={canFinalizeSelected ? 'Finalize' : 'ต้องมีผู้รับผิดชอบ, Competency และสัดส่วนรวม 100%'}
                                    >
                                        <Lock size={16} />
                                        Finalize
                                    </button>
                                    <button
                                        ...
                                        onClick={() => setConfirmAction({ type: 'delete', session: selectedSession })}
                            ...
                            <div className="activity-session-summary">
                                <span><UserPlus size={14} /> ผู้รับผิดชอบ {selectedSession.assignmentCount} คน</span>
                                <span><Award size={14} /> Competency {selectedSession.competencyCount} รายการ</span>
                                <span>Mapping รวม {selectedSession.competencyPercentTotal}%</span>
                                <span>ลงทะเบียน {selectedSession.registrationCount} รายการ</span>
                            ...
                            <div className="activity-session-tabs">
                                {TAB_OPTIONS.map(tab => (
                                    <button
                                        key={tab.id}
                                        type="button"
                                        className={activeTab === tab.id ? 'activity-session-tab activity-session-tab--active' : 'activity-session-tab'}
                            ...
                            <div className="activity-session-tab-panel">
                                {detailLoading ? (
                                    <div className="activity-session-empty">กำลังโหลดรายละเอียด...</div>
                                ) : activeTab === 'info' ? (
                                    <>
                                        <SessionForm form={sessionForm} setForm={setSessionForm} disabled={sessionReadOnly || submitting} />
                                        <div className="activity-session-panel-actions">
                                            {sessionReadOnly && <span>รอบกิจกรรมนี้อยู่ในสถานะที่แก้ไขไม่ได้</span>}
                                            <button className="course-btn course-btn--primary" onClick={handleSaveInfo} disabled={sessionReadOnly || submitting}>
                                                <Save size={16} />
                                                บันทึกข้อมูลรอบ
                                            </button>
                                        </div>
                                    ...
                                    <div className="activity-session-table-panel">
                                        <div className="activity-session-panel-toolbar">
                                            ...
                                            <button className="course-btn course-btn--ghost" onClick={addAssignmentRow} disabled={sessionReadOnly || submitting}>
                                                <Plus size={16} />
                                                เพิ่มผู้รับผิดชอบ
                                            </button>
                                        </div>
                                        <div className="activity-session-table-wrap">
                                            <table className="activity-session-table">
                                                ...
                                                <tbody>
                                                    {assignmentRows.length ? assignmentRows.map((row, index) => (
                                                        <tr key={row.rowId}>
                                                            <td>
                                                                <select
                                                                    className="course-input"
                                                                    value={row.userId}
                                                                    onChange={event => setAssignmentRows(prev => prev.map((item, itemIndex) => (
                                                            ...
                                                            <td>
                                                                <select
                                                                    ...
                                                                    onChange={event => setAssignmentRows(prev => prev.map((item, itemIndex) => (
                                                                        itemIndex === index ? { ...item, assignmentRole: event.target.value } : item
                                                                    )))}
                                                                    disabled={sessionReadOnly || submitting}
                                                                >
                                                            ...
                                                            <td>
                                                                <div className="activity-session-permission-list">
                                                                    <label><input type="checkbox" checked={row.canRecordAttendance} onChange={event => setAssignmentRows(prev => prev.map((item, itemIndex) => (itemIndex === index ? { ...item, canRecordAttendance: event.target.checked } : item)))} disabled={sessionReadOnly || submitting} /> เช็กชื่อ</label>
                                                                    <label><input type="checkbox" checked={row.canGrade} onChange={event => setAssignmentRows(prev => prev.map((item, itemIndex) => (itemIndex === index ? { ...item, canGrade: event.target.checked } : item)))} disabled={sessionReadOnly || submitting} /> ให้คะแนน</label>
                                                                    <label><input type="checkbox" checked={row.canFinalize} onChange={event => setAssignmentRows(prev => prev.map((item, itemIndex) => (itemIndex === index ? { ...item, canFinalize: event.target.checked } : item)))} disabled={sessionReadOnly || submitting} /> Finalize</label>
                                                                </div>
                                                            </td>
                                                            <td>
                                                                <input
                                                            ...
                                                            <td>
                                                                <button
                                                                    ...
                                                                    onClick={() => setAssignmentRows(prev => prev.filter((_, itemIndex) => itemIndex !== index))}
                                                                    disabled={sessionReadOnly || submitting}
                                                        ...
                                                        <tr>
                                                            <td colSpan={5} className="activity-session-table-empty">ยังไม่มีผู้รับผิดชอบ</td>
                                                        </tr>
                                                    )}
                                                </tbody>
                                            </table>
                                        ...
                                        <div className="activity-session-panel-actions">
                                            <button className="course-btn course-btn--primary" onClick={handleSaveAssignments} disabled={sessionReadOnly || submitting}>
                                                <Save size={16} />
                                                บันทึกผู้รับผิดชอบ
                                            </button>
                                        </div>
                                    ...
                                    <div className="activity-session-table-panel">
                                        <div className="activity-session-panel-toolbar">
                                            <div>
                                                ...
                                                <span className={Math.abs(competencyTotal - 100) < 0.0001 ? 'activity-session-total-ok' : 'activity-session-total-warn'}>
                                                    รวม {competencyTotal}% / 100%
                                                </span>
                                            </div>
                                            <button className="course-btn course-btn--ghost" onClick={addCompetencyRow} disabled={sessionReadOnly || submitting}>
                                                <Plus size={16} />
                                                เพิ่ม Competency
                                            </button>
                                        </div>
                                        <div className="activity-session-table-wrap">
                                            <table className="activity-session-table">
                                                ...
                                                <tbody>
                                                    {competencyRows.length ? competencyRows.map((row, index) => (
                                                        <tr key={row.rowId}>
                                                            <td>
                                                                <select
                                                                    className="course-input"
                                                                    value={row.competencyId}
                                                                    onChange={event => setCompetencyRows(prev => prev.map((item, itemIndex) => (
                                                                        itemIndex === index ? { ...item, competencyId: event.target.value } : item
                                                                    )))}
                                                                    disabled={sessionReadOnly || submitting}
                                                                >
                                                            ...
                                                            <td>
                                                                <input
                                                                    ...
                                                                    onChange={event => setCompetencyRows(prev => prev.map((item, itemIndex) => (
                                                                        itemIndex === index ? { ...item, maxPercent: event.target.value } : item
                                                                    )))}
                                                                    disabled={sessionReadOnly || submitting}
                                                                />
                                                            </td>
                                                            <td>
                                                                <button
                                                                    className="course-icon-btn course-icon-btn--danger"
                                                                    onClick={() => setCompetencyRows(prev => prev.filter((_, itemIndex) => itemIndex !== index))}
                                                                    disabled={sessionReadOnly || submitting}
                                                                    title="ลบ Competency"
                                                                >
                                                                    <Trash2 size={16} />
                                                        ...
                                                        <tr>
                                                            <td colSpan={3} className="activity-session-table-empty">ยังไม่มี Competency mapping</td>
                                                        </tr>
                                                    )}
                                                </tbody>
                                            </table>
                                        ...
                                        <div className="activity-session-panel-actions">
                                            ...
                                            <button className="course-btn course-btn--primary" onClick={handleSaveCompetencies} disabled={sessionReadOnly || submitting}>
                                                <Save size={16} />
                                                บันทึก Competency
                                            </button>
                                        </div>
            ...
            {createModalOpen && (
                <BaseModal
                    ...
                    footer={(
                        <>
                            <button className="course-btn course-btn--ghost" onClick={() => setCreateModalOpen(false)} disabled={submitting}>
                                ยกเลิก
                            </button>
                            <button className="course-btn course-btn--primary" onClick={handleCreateSession} disabled={submitting || !createForm.startAt || !createForm.endAt}>
                                <Save size={16} />
                ...
                >
                    <SessionForm form={createForm} setForm={setCreateForm} disabled={submitting} />
                </BaseModal>
            )}
            ...
            {confirmConfig && (
                <ConfirmActionModal
                    ...
                    confirmLabel={submitting ? 'กำลังดำเนินการ...' : confirmConfig.confirmLabel}
