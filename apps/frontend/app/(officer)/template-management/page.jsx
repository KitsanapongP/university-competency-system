...
import {
  ...
  fetchTemplateCompetencies,
  updateTemplateCompetencies,
  createTemplate,
  saveTemplateItems,
  previewDuplicateTemplate,
  duplicateTemplate,
} from "../../../lib/template";
...
function normalizeCompetencies(competencies) {
  return (Array.isArray(competencies) ? competencies : [])
    ...
    .map((competency, index) => {
      ...
      const nameTh =
        competency.name_th || competency.nameTh || competency.name || "";
...
function insertChild(cats, parentId, newChild) {
  return cats.map((c) => {
    ...
    if (c.children?.length)
      return { ...c, children: insertChild(c.children, parentId, newChild) };
    return c;
  });
}
function renameCategory(cats, id, name) {
  return cats.map((c) => {
    ...
    if (c.children?.length)
      return { ...c, children: renameCategory(c.children, id, name) };
    return c;
  });
}
...
function getNextCode(parentCode, siblings) {
  const nextIndex =
    (siblings || []).reduce((highest, sibling) => {
...
function getSubtreeDepth(category) {
  ...
  return Math.max(
    ...category.children.map((child) => 1 + getSubtreeDepth(child)),
  );
}
...
function moveCategory(cats, categoryId, targetParentId = null) {
  ...
  const moved = recodeCategorySubtree(
    moving,
    getNextCode(targetParent.code, siblings),
  );
  return insertChild(withoutMoving, targetParentId, moved);
}
function moveCourseToCategory(
  coursesByCategory,
  course,
  targetCategoryId,
  beforeCourseId = null,
  ...
  if (!course?.id || !sourceCategoryId || !targetCategoryId)
    return coursesByCategory;

  const next = { ...coursesByCategory };
  const sourceCourses = [...(next[sourceCategoryId] || [])].filter(
    (item) => item.id !== course.id,
  );
  const targetCourses =
    sourceCategoryId === targetCategoryId
      ? sourceCourses
      : [...(next[targetCategoryId] || [])].filter(
          (item) => item.id !== course.id,
        );
  const insertionIndex = beforeCourseId
    ? Math.max(
        0,
        targetCourses.findIndex((item) => item.id === beforeCourseId),
      )
    : targetCourses.length;

  targetCourses.splice(insertionIndex, 0, {
    ...course,
...
export default function TemplateManagementPage() {
  ...
  const [competencyManagerLoading, setCompetencyManagerLoading] =
    useState(false);
  const [competencyManagerSaving, setCompetencyManagerSaving] = useState(false);
  const [competencyRemovalConfirmation, setCompetencyRemovalConfirmation] =
    useState(null);
  ...
  const showAlert = useCallback(
    (message, { title, details, type = "warning" } = {}) => {
      setAlertModal({
        open: true,
        title: title || "",
        message,
  ...
  const templateCompetencyMessage = useCallback(
    (error) => {
      const messages = {
        TEMPLATE_ACTIVE: {
      ...
      return (
        ...
        error?.message ||
        (language === "th"
          ? "ไม่สามารถจัดการสมรรถนะได้"
          : "Unable to manage competencies.")
      );
    },
    [language],
  );
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const actualTmplData = Array.isArray(tmplData)
          ? tmplData
          : tmplData?.data || [];
            ...
            isActive:
              t.is_active !== undefined
                ...
                : t.isActive !== undefined
                  ...
                  : false,
            totalCourseCount:
              t.total_course_count !== undefined
                ? t.total_course_count
                : t.TotalCourseCount !== undefined
                  ? t.TotalCourseCount
                  : t.mapped_course_count || 0,
            masterData: t.curriculum_name_th
              ? {
                  id: t.curriculum_id,
                  name: t.curriculum_name_th,
                  code: t.curriculum_code,
                }
              : t.masterData,
          }));
          setTemplates(mapped);
  ...
  const currentCategories = selectedTemplate
    ...
    : [];
  const currentCoursesByCat = selectedTemplate
    ? coursesByTemplate[selectedTemplate.id] || {}
    : {};
  const currentWeightsByCourse = selectedTemplate
    ? weightsByTemplate[selectedTemplate.id] || {}
    : {};
  const currentCompetencies = useMemo(
    () =>
      selectedTemplate ? competenciesByTemplate[selectedTemplate.id] || [] : [],
    [selectedTemplate, competenciesByTemplate],
  );

  const updateCurrentCategories = useCallback(
    (updater) => {
  ...
  const courseCountMap = useMemo(() => {
    const m = {};
    templates.forEach((t) => {
      const loadedCount = Object.values(coursesByTemplate[t.id] || {}).reduce(
        (s, a) => s + a.length,
        0,
      );
      m[t.id] =
      ...
      .then(([detail, struct]) => {
        ...
        function convertCats(cats, courseMap) {
          return (cats || []).map((cat, i) => {
            const stableId =
              typeof cat.id === "number" && cat.id < 5000
                ? cat.id
                : typeof cat.category_id === "number" && cat.category_id < 5000
                  ? cat.category_id
                  : ++idRef.current;
            const converted = {
              id: stableId,
            ...
            if (cat.courses?.length) {
              courseMap[stableId] = cat.courses.map((c) => ({
                ...
                courseId:
                  c.courseId || c.course_id || c.id || courseIdRef.current,
                code: c.code,
        ...
        const cats =
          detail && detail.categories
            ? convertCats(detail.categories, courseMap)
            : [];
        ...
        (struct?.custom_courses || []).forEach((course) => {
          ...
          if (serverCourseId)
            customCourseIds.set(serverCourseId, localCourseId);
        ...
        if (struct && struct.items) {
          struct.items.forEach((it) => {
            if (
              ...
              const courseId =
                customCourseIds.get(it.course_id) || it.course_id;
        ...
        const tplComps =
          struct && struct.competencies && struct.competencies.length > 0
            ? struct.competencies.map((sc, idx) => {
                const master = allCompetencies.find(
                  (c) => c.id === (sc.id || sc.competency_id),
                );
                return (
                  master || {
  ...
  const handleRequestDuplicateTemplate = useCallback(
    async (template) => {
      ...
      try {
        ...
        setDuplicateCompetencyIds(
          (Array.isArray(competencies) ? competencies : [])
            .map((comp) => comp.id || comp.competency_id)
            .filter(Boolean),
        );
        ...
        showTemplateToast(
          "error",
          language === "th"
            ? "ไม่สามารถโหลด Competency ของแบบแผนต้นฉบับได้"
            : "Unable to load source competencies.",
          error?.message,
        );
      }
    },
  ...
  const handlePreviewDuplicate = useCallback(
    async (payload) => {
      if (!duplicateSource?.id) {
        throw new Error(
          language === "th"
            ? "ไม่พบแบบแผนต้นฉบับ"
            : "Source assessment plan was not found.",
        );
      }
  ...
  const handleSaveDuplicate = useCallback(
    async (payload) => {
      ...
      try {
        ...
        if (!createdID)
          throw new Error("duplicate template returned no template id");

        const nextTemplate = {
          ...actualCreated,
          id: createdID,
          name: actualCreated?.name || payload.name,
          curriculum_id: actualCreated?.curriculum_id || payload.curriculum_id,
          academicYear:
            actualCreated?.version_year_be ||
            duplicateSource?.academicYear ||
            "",
          isActive: actualCreated?.is_active ?? false,
          masterData: {
            id: payload.curriculum_id,
            name:
              actualCreated?.curriculum_name_th ||
              duplicateSource?.masterData?.name ||
              "",
            nameTh:
              actualCreated?.curriculum_name_th ||
              duplicateSource?.masterData?.nameTh ||
              "",
            year:
              actualCreated?.version_year_be ||
              duplicateSource?.academicYear ||
        ...
        showTemplateToast(
          "success",
          language === "th"
            ? "สร้างสำเนาแบบแผนสำเร็จ"
            : "Assessment plan duplicated successfully.",
        );
      } catch (error) {
        showTemplateToast(
          "error",
          language === "th"
            ? "ไม่สามารถสร้างสำเนาแบบแผนได้"
            : "Unable to duplicate assessment plan.",
          error?.message,
        );
      }
    },
  ...
  const handleSaveTitle = useCallback(async () => {
    ...
    if (trimmed && selectedTemplate && trimmed !== selectedTemplate.name) {
      try {
        ...
        setTemplates((p) =>
          p.map((t) =>
            t.id === selectedTemplate.id ? { ...t, name: trimmed } : t,
          ),
        );
        setSelectedTemplate((p) => ({ ...p, name: trimmed }));
      } catch (err) {
        showAlert(
          "ไม่สามารถเปลี่ยนชื่อแบบแผนการประเมินได้: " +
            (err.message || "เกิดข้อผิดพลาด"),
          { type: "error", title: "เกิดข้อผิดพลาด" },
        );
      }
    }
  ...
  const handleConfirmDeleteTemplate = useCallback(async () => {
    ...
    try {
      ...
      showAlert(err.message || "ไม่สามารถลบแบบแผนการประเมินได้", {
        type: "error",
  ...
  const handleSaveTemplate = useCallback(
    async ({ name, masterData, competencyIds }) => {
      let createdId = null;
      ...
      try {
        ...
        if (!createdId)
          throw new Error("template creation returned no template id");
      } catch (err) {
        console.error("Create template API failed:", err);
        showAlert(err.message || "Unable to create assessment plan.", {
          type: "error",
          title: "Assessment plan was not created",
        });
        return;
      }
      ...
      const newTemplate = {
        ...
        name,
        totalCourseCount:
          actualCreated?.total_course_count ||
          actualCreated?.TotalCourseCount ||
          masterData?.total_courses ||
          masterData?.course_count ||
          0,
        masterData: masterData
          ? {
              ...
              name:
                masterData.name ||
                masterData.nameTh ||
                masterData.curriculum_name_th ||
                "",
            }
          : null,
        isActive:
          typeof actualCreated?.is_active === "boolean"
            ? actualCreated.is_active
            : typeof actualCreated?.isActive === "boolean"
              ? actualCreated.isActive
              : false,
      };
      ...
      if (masterData) {
        ...
        if (
          !currentMaster.categories ||
          currentMaster.categories.length === 0
        ) {
          try {
            ...
            if (detail) {
              currentMaster = {
                ...currentMaster,
                ...detail,
                name: detail.nameTh || detail.name || currentMaster.nameTh,
              };
            }
          } catch (err) {
  ...
  // Auto Save Helper
  // ============================================================
  const triggerAutoSaveToAPI = useCallback(
    (tplId, nextCats, nextCoursesMap, nextWeightsMap) => {
      if (!tplId) return;
  ...
  const loadTemplateCompetencyManager = useCallback(
    async (templateID) => {
      ...
      try {
        ...
        showTemplateToast(
          "error",
          templateCompetencyMessage(error),
          error.message,
        );
        return null;
      } finally {
  ...
  const handleOpenCompetencyManager = useCallback(async () => {
    ...
    const managerData = await loadTemplateCompetencyManager(
      selectedTemplate.id,
    );
  ...
  const handleReloadEditor = useCallback(async () => {
    ...
    try {
      ...
      showTemplateToast(
        "success",
        language === "th"
          ? "โหลดข้อมูลแบบแผนการประเมินใหม่แล้ว"
          : "Assessment plan data reloaded.",
      );
    } catch (error) {
      showTemplateToast(
        "error",
        language === "th"
          ? "โหลดข้อมูลแบบแผนการประเมินไม่สำเร็จ"
          : "Unable to reload assessment plan data.",
        error?.message,
      );
    }
  }, [
  ...
  const applyTemplateCompetencySelection = useCallback(
    async (competencyIds, confirmRemoval = false) => {
      if (!selectedTemplate) return;
      ...
      try {
        ...
        const updatedCompetencies = normalizeCompetencies(
          result?.competencies || [],
        );
        setCompetenciesByTemplate((current) => ({
          ...current,
          [selectedTemplate.id]: updatedCompetencies,
        }));
        refreshTemplateStructure(selectedTemplate.id);
        ...
        showTemplateToast(
          "success",
          language === "th"
            ? "บันทึกสมรรถนะของแบบแผนการประเมินแล้ว"
            : "Assessment plan competencies saved.",
        );
      } catch (error) {
    ...
    [
      ...
      selectedTemplate,
      showTemplateToast,
      templateCompetencyMessage,
    ],
  );

  const handleSaveTemplateCompetencies = useCallback(
    (competencyIds) => {
  ...
  const handleConfirmCompetencyRemoval = useCallback(() => {
    ...
    applyTemplateCompetencySelection(
      competencyRemovalConfirmation.competencyIds,
      true,
    );
  ...
  const handleCreateCategory = useCallback(
    (parentId = selectedCategory?.id ?? null) => {
      ...
      if (selectedTemplate.isActive) {
        showAlert(
          "ไม่สามารถเพิ่มหมวดวิชาได้ในขณะที่ Template มีสถานะพร้อมใช้งาน (Active)",
          {
            type: "warning",
            title: "Template อยู่ในสถานะ Active",
            details: "กรุณาเปลี่ยนสถานะเป็นปิดใช้งานก่อนแก้ไข",
          },
        );
        return;
      }
      ...
      if (!parentId) {
        ...
        const code = getNextCode(
          "",
          getDirectChildren(currentCategories, null),
        );
        const newCat = {
          id: newId,
          code,
        ...
        if (getDepthFromCode(parent.code) >= 3) {
          showAlert("ไม่สามารถสร้างหมวดวิชาที่ลึกกว่า 4 ระดับได้", {
            type: "info",
            title: "ถึงระดับสูงสุดแล้ว",
          });
          return;
        }
        ...
        if (parent.fromMaster && parentCourses.length > 0) {
          showAlert(
            `หมวด "${parent.code} ${parent.name}" มีรายวิชาอยู่แล้ว ไม่สามารถสร้างหมวดย่อยได้`,
            { type: "warning", title: "ไม่สามารถสร้างหมวดย่อยได้" },
          );
          return;
        }
        const code = getNextCode(
          parent.code,
          getDirectChildren(currentCategories, parentId),
        );
        const newCat = {
          id: newId,
          code,
  ...
  const canMoveTemplateCategory = useCallback(
    (targetCategory = null) => {
      if (!draggingCategoryId || !selectedTemplate || selectedTemplate.isActive)
        return false;
      ...
      if (
        targetCategory.id === movingCategory.id ||
        isDescendantOf(movingCategory, targetCategory.id)
      )
        return false;
      if ((currentCoursesByCat[targetCategory.id] || []).length > 0)
        return false;
      return (
        getDepthFromCode(targetCategory.code) +
          1 +
          getSubtreeDepth(movingCategory) <=
        3
      );
    },
    [
  ...
  const handleTemplateCategoryDragStart = useCallback(
    (event, category) => {
  ...
  const handleTemplateCategoryDragOver = useCallback(
  ...
  const handleTemplateCategoryRootDragOver = useCallback(
    (event) => {
  ...
  const commitTemplateCategoryMove = useCallback(
    (targetCategory = null) => {
      ...
      setSelectedCategory((current) =>
        current ? findById(nextCategories, current.id) || null : null,
      );
      triggerAutoSaveToAPI(
        selectedTemplate.id,
        nextCategories,
  ...
  const handleTemplateCategoryDrop = useCallback(
    (event, targetCategory) => {
      event.preventDefault();
  ...
  const handleTemplateCategoryRootDrop = useCallback(
    (event) => {
      event.preventDefault();
  ...
  const handleTemplateCourseDragStart = useCallback(
    (event, course) => {
  ...
  const moveTemplateCourse = useCallback(
    (targetCategory, beforeCourseId = null) => {
      if (
      ...
      if (
        draggedTemplateCourse.fromMaster ||
  ...
  const handleTemplateCourseCategoryDragOver = useCallback(
    (event, targetCategory) => {
      if (
        !draggedTemplateCourse ||
        selectedTemplate?.isActive ||
  ...
  const handleTemplateCourseCategoryDrop = useCallback(
    (event, targetCategory) => {
      event.preventDefault();
  ...
  const handleTemplateCourseMove = useCallback(
    ({ targetCategory, beforeCourseId }) => {
  ...
  const handleRequestDeleteCategory = useCallback(
    (cat) => {
      if (selectedTemplate?.isActive) {
        showAlert(
          "ไม่สามารถลบหมวดวิชาได้ในขณะที่ Template มีสถานะพร้อมใช้งาน (Active)",
          {
            type: "warning",
            title: "Template อยู่ในสถานะ Active",
            details: "กรุณาเปลี่ยนสถานะเป็นปิดใช้งานก่อนแก้ไข",
  ...
  const handleConfirmDeleteCategory = useCallback(() => {
    if (!deletingCategory || !selectedTemplate || selectedTemplate.isActive)
      return;
    ...
    setCoursesByTemplate((p) => ({
      ...p,
      [selectedTemplate.id]: nextCoursesMap,
    }));
    setWeightsByTemplate((p) => ({
      ...p,
      [selectedTemplate.id]: nextWeightsMap,
    }));
    const nextCats = removeCategory(currentCategories, deletingCategory.id);
    updateCurrentCategories(nextCats);
    if (
      selectedCategory?.id === deletingCategory.id ||
      isDescendantOf(deletingCategory, selectedCategory?.id)
    ) {
      setSelectedCategory(null);
    }
  ...
  const handleAddCourse = useCallback(
    (catId, data) => {
      ...
      if (selectedTemplate.isActive) {
        showAlert(
          "ไม่สามารถเพิ่มรายวิชาได้ในขณะที่ Template มีสถานะพร้อมใช้งาน (Active)",
          {
            type: "warning",
            title: "Template อยู่ในสถานะ Active",
            details: "กรุณาเปลี่ยนสถานะเป็นปิดใช้งานก่อนแก้ไข",
      ...
      const nextCoursesMap = {
        ...currentCoursesByCat,
        [catId]: [...(currentCoursesByCat[catId] || []), course],
      };
  ...
  const handleUpdateCourse = useCallback(
    (catId, updatedCourse) => {
      if (
        !selectedTemplate ||
        selectedTemplate.isActive ||
        updatedCourse?.fromMaster
      )
        return false;
      const nextCoursesMap = {
        ...currentCoursesByCat,
        [catId]: (currentCoursesByCat[catId] || []).map((c) =>
  ...
  const handleRequestDeleteCourse = useCallback(
    (catId, course) => {
      if (course?.fromMaster) return;
      if (selectedTemplate?.isActive) {
        showAlert(
          "ไม่สามารถลบรายวิชาได้ในขณะที่ Template มีสถานะพร้อมใช้งาน (Active)",
          {
            type: "warning",
            title: "Template อยู่ในสถานะ Active",
            details: "กรุณาเปลี่ยนสถานะเป็นปิดใช้งานก่อนแก้ไข",
  ...
  const handleBulkDeleteTemplateCourses = useCallback(
    (catId, courses) => {
      if (!selectedTemplate || selectedTemplate.isActive || !courses?.length)
        return false;
      const deletingIds = new Set(
        courses
          .filter((course) => !course.fromMaster)
      ...
      const nextCoursesMap = {
        ...currentCoursesByCat,
        [catId]: (currentCoursesByCat[catId] || []).filter(
          (course) => !deletingIds.has(course.id),
        ),
      };
      const nextWeightsMap = { ...currentWeightsByCourse };
  ...
  const handleRequestBulkDeleteTemplateCourses = useCallback(
    (catId, courses) => {
      if (!selectedTemplate || selectedTemplate.isActive || !courses?.length)
        return false;
  ...
  const validateTemplateCourseBeforeSave = useCallback(
    (candidate) => {
      ...
      alert(
        `ไม่สามารถบันทึกรายวิชาได้ เพราะมีรหัสวิชา ${candidate.code} อยู่ใน Template นี้แล้ว`,
      );
      return false;
    },
    [currentCoursesByCat],
  ...
  const handleConfirmDeleteCourse = useCallback(() => {
    if (!deletingCourse || !selectedTemplate || selectedTemplate.isActive)
      return;
    const catId = deletingCourse._catId;
    const nextCoursesMap = {
      ...currentCoursesByCat,
      [catId]: (currentCoursesByCat[catId] || []).filter(
        (c) => c.id !== deletingCourse.id,
      ),
    };
    updateCurrentCourses(nextCoursesMap);
    const nextWeightsMap = { ...currentWeightsByCourse };
  ...
  const handleSetWeight = useCallback(
    (courseId, compId, weight) => {
      if (!selectedTemplate) return;
      const course = {
        ...(currentWeightsByCourse[courseId] || {}),
        [compId]: weight,
      };
  ...
  const handleToggleTemplateStatus = useCallback(
    async (newStatus) => {
      if (!selectedTemplate) return;
      ...
      if (newStatus === true) {
        ...
        if (allCourses.length === 0) {
          showAlert(
            "ไม่สามารถเปิดใช้งานได้เนื่องจาก Template ยังไม่มีรายวิชา",
            { type: "error", title: "ไม่สามารถเปิดใช้งานได้" },
          );
          return;
        }
        if (comps.length === 0) {
          showAlert(
            "ไม่สามารถเปิดใช้งานได้เนื่องจาก Template ยังไม่มีสมรรถนะ",
            { type: "error", title: "ไม่สามารถเปิดใช้งานได้" },
          );
          return;
        }
        ...
        const coursesWithoutComp = allCourses.filter((c) => {
          ...
          return !comps.some(
            (comp) => (Number(courseWeights[comp.id]) || 0) > 0,
          );
        });
        ...
        if (coursesWithoutComp.length > 0) {
          ...
          const names = coursesWithoutComp
            ...
            .map(
              (c) =>
                `  - ${c.code || "(ไม่มีรหัส)"} ${c.nameTh || c.name || ""}`,
            )
            .join("\n");
          const extra =
            coursesWithoutComp.length > maxShow
      ...
      try {
        ...
        showAlert(err.message || "เกิดข้อผิดพลาดในการเปลี่ยนสถานะ", {
          type: "error",
      ...
      <div className="tm-page">
        ...
        <div className="tm-header">
          <div>
            ...
            <p className="tpl-list-view__sub">
              เลือกแบบแผนการประเมินที่ต้องการแก้ไข หรือสร้างแบบแผนใหม่
            </p>
          </div>
        ...
        {templates.length === 0 ? (
          <div className="tpl-list-view__empty">
            ...
            <p>
              ยังไม่มีแบบแผนการประเมิน — กดปุ่ม &quot;สร้างแบบแผนใหม่&quot;
              เพื่อเริ่ม
            </p>
            <button
              className="btn btn--primary"
          ...
          <div className="tpl-card-grid">
            ...
            {/* + Create card */}
            <div
              className="tpl-card tpl-card--create"
              onClick={handleOpenCreateTemplate}
            >
        ...
        {showTemplateModal && (
          <TemplateFormModal
            ...
            duplicateSource={
              templateModalMode === "duplicate" ? duplicateSource : null
            }
            duplicateCompetencyIds={
              templateModalMode === "duplicate" ? duplicateCompetencyIds : []
            }
        ...
        <ToastNotifications
          ...
          onCloseSuccess={() =>
            setToast((current) => ({ ...current, success: "" }))
          }
          onCloseError={() =>
    ...
    <div className="tm-page tm-page--editor">
      <div className="editor-topbar">
        <div className="editor-topbar__main">
          <button
            className="btn btn--ghost btn--sm editor-topbar__back"
        ...
        <div className="editor-topbar__actions">
          ...
          >
            <Copy size={14} /> {editorCopy.duplicate}
          </button>
          <button
            className="btn btn--primary btn--sm"
            onClick={handleOpenCompetencyManager}
          >
      ...
      <div className="editor-tabs">
        {[
          {
            id: "setup",
            label: editorCopy.setup,
            icon: <Settings size={14} />,
          },
          {
            id: "weight",
            label: editorCopy.weight,
            icon: <SlidersHorizontal size={14} />,
          },
          {
            id: "overview",
            label: editorCopy.overview,
      ...
      {deletingCourse && (
        <ConfirmActionModal
          open
          title={
            language === "th" ? "ยืนยันการลบรายวิชา" : "Confirm course deletion"
          }
          message={
            language === "th" ? (
              <>
                ยืนยันการลบรายวิชา{" "}
                <strong>{deletingCourse.code || deletingCourse.nameTh}</strong>{" "}
                หรือไม่?
              </>
            ) : (
              <>
          ...
          hint={
            language === "th"
              ? "รายวิชาที่ลบแล้วจะไม่สามารถกู้คืนได้"
      ...
      {deletingCourses && (
        <ConfirmActionModal
          ...
          title={
            language === "th"
              ? "ยืนยันการลบรายวิชาที่เลือก"
              : "Confirm deleting selected courses"
          }
          message={
            language === "th"
          ...
          variant="danger"
          onConfirm={() =>
            handleBulkDeleteTemplateCourses(
              deletingCourses.catId,
              deletingCourses.courses,
            )
          }
      ...
      <ConfirmActionModal
        ...
        impact={
          competencyRemovalConfirmation?.impacts?.length ? (
            <ul className="template-competency-manager__impact-list">
              {competencyRemovalConfirmation.impacts.map((impact) => (
                <li key={impact.competency_id}>
                  <strong>{impact.code}</strong> {impact.name_th} (
                  {impact.mapping_count}{" "}
                  {language === "th" ? "รายวิชา" : "courses"})
                </li>
              ))}
            </ul>
          ) : null
        ...
        onCancel={() =>
          !competencyManagerSaving && setCompetencyRemovalConfirmation(null)
        }
      />
      <ToastNotifications
        success={toast.success}
