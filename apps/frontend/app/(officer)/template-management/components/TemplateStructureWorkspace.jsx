...
function getWorkspaceCopy(language) {
  if (language === "en") {
    return {
      ...
      manageLeafCourses:
        "Add and manage Template-only courses in this category",
      aggregateCategory:
        "This category shows courses from its subcategories. Add courses only to the deepest category.",
      allCoursesReadOnly:
        "All courses are shown here for review. This view is read-only.",
      activeReadOnly:
        "This assessment plan is active, so its structure is read-only.",
      setupHint: "Select a category to manage Template-only courses",
      addCourse: "Add course",
      addCategory: "Add category",
      structureTitle: "Curriculum structure",
      emptyStructure:
        "Add a category to begin building the assessment plan structure",
      templateOnly: "Template only",
      setupActiveWarning: "so its additional structure cannot be edited.",
      courseInformationRequired:
        "Complete the course information before setting a weight",
    };
  }
    ...
    <div className="template-weight-editor">
      <div className="template-weight-editor__toolbar">
        ...
        <div className="template-weight-editor__actions">
          <div
            className="template-weight-editor__course-summary"
            aria-label={`${copy.courseCount} ${courseCount} ${copy.totalCredits} ${totalCredits}`}
          >
            <span>
              <BookOpen size={14} /> {copy.courseCount} {courseCount}
            </span>
            <span>
              {copy.totalCredits} <strong>{totalCredits}</strong>
            </span>
          </div>
          {action && (
            <button
      ...
      {summary && (
        <div className="template-weight-editor__summary-strip">{summary}</div>
      )}
...
function TemplateWeightEditor({
  ...
  const renderWeightCells = (course) => [
    <td key="course-type" className="ss-cell ss-cell--weight">
      <span
        className={`template-course-type-badge ${course.isCoreCourse ? "template-course-type-badge--required" : "template-course-type-badge--elective"}`}
      >
        {course.isCoreCourse
          ? language === "en"
            ? "Required"
    ...
    ...competencies.map((competency) => {
      ...
      return (
        <td
          ...
          style={{ "--wc": competency.color }}
        >
          <div
            className={`ss-weight-wrap ${isMapped ? "ss-weight-wrap--active" : ""} ${isLocked ? "ss-weight-wrap--locked" : ""}`}
          >
            <input
              ...
              title={
                isLocked
                  ? copy.courseInformationRequired
                  : `${language === "en" ? competency.nameEn || competency.nameTh || competency.name : competency.nameTh || competency.nameEn || competency.name} weight`
              }
    ...
    return (
      <div className="tm-panel template-structure-workspace">
        <div className="panel-empty">
          {language === "en"
            ? "Select an assessment plan first"
            : "เลือกแบบแผนการประเมินก่อน"}
        </div>
      </div>
    );
  }
      ...
      {isSetupMode && template.isActive && (
        <div className="template-structure-workspace__active-warning">
          ...
          <span>
            <strong>
              {language === "en"
                ? "Assessment plan is active"
                : "แบบแผนพร้อมใช้งานแล้ว"}
            </strong>{" "}
            {copy.setupActiveWarning}
          </span>
        </div>
      )}
        ...
        <div className="template-structure-workspace__sidebar">
          <CurriculumStructureSidebar
            ...
            getCategoryBadge={(category) =>
              category.fromMaster ? "" : copy.templateOnly
            }
            getCourseBadge={(course) =>
              course.fromMaster ? "" : copy.templateOnly
            }
          />
        </div>
          ...
          {isSetupMode ? (
            ...
            >
              <CurriculumCourseEditorPanel
                ...
                isLeafCategory={
                  !showAllCourses && isLeafCategory(selectedCategory)
                }
                ...
                canEditCategoryMetadata={Boolean(
                  selectedCategory &&
                    !selectedCategory.fromMaster &&
                    isSetupMode &&
                    !isAggregateCategoryView,
                )}
                canManageCourses={isSetupMode}
                allowSelection={!isAggregateCategoryView}
                getCourseCapabilities={getEditorCourseCapabilities}
                onRenameCategory={(categoryId, updates) =>
                  onRenameCategory?.(
                    categoryId,
                    updates.nameTh ?? updates.name ?? "",
                  )
                }
                onDeleteCategory={onDeleteCategory}
                onAddCourse={(course) =>
                  onAddCourse?.(selectedCategory?.id, course)
                }
                onUpdateCourse={(course) =>
                  onUpdateCourse?.(selectedCategory?.id, course)
                }
                onDeleteCourse={(course) =>
                  onDeleteCourse?.(selectedCategory?.id, course)
                }
                onBulkDeleteCourses={(courses) =>
                  onBulkDeleteCourses?.(selectedCategory?.id, courses)
                }
                onMoveCourse={onMoveCourse}
                onValidateCourse={onValidateCourse}
                onCourseDragStart={(event, course) =>
                  onCourseDragStart?.(event, {
                    ...course,
